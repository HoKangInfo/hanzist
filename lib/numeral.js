...
var koliPoint = (str) => (str || '').replace(/([千百十])/g, '$1,').split(',')
    .filter((value) => value).map((value) => {
        ...
        let multed = unit(value[0]),
            mult = unit(value.slice(-1));
            mult = (value.length !== 1 && mult % 10) ? NaN : mult;
        ...
        switch (value.length) {
            ...
            case 2:
                ...
                break;
            case 3:
                multed = multed ? NaN : unit(value[1]);
                break;
            default:
                multed = NaN;
                break;
...
var toNumber = function(str) {
    ...
    let hanNum = normalize(str.trim()).replace(/(\d+)/g, (match) => toHanNumber(match));
